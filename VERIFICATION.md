...
- **Unit / integration (26 tests, CI-gated):** `Model.js` pure functions —
  schedule parsing, countdown, live-window detection, leaderboard joins, gap
  formatting, safety-car/flag fold, and the input sanitizer — all against real
  captured jolpica + openf1 responses under `tests/fixtures/`.
